  ...
  <div class="bg-[#ffffff] dark:bg-gray-800 min-h-screen py-[60px]">
      ...
      <client-only>
        ...
        <div class="flex flex-row items-center gap-2">
          ...
          <nuxt-link class="text-sm text-slate-600 font-bold dark:text-white" href="/music" title="音乐搜索小助手">
            <img v-if="colorMode.preference === 'light'" class="w-[20px] h-[20px]" src="@/assets/theme/music-dark.svg" alt="">
            <img v-if="colorMode.preference === 'dark'" class="w-[20px] h-[20px]" src="@/assets/theme/music-light.svg" alt="">
          </nuxt-link>
        </div>
      </client-only>
    </div>
    ...
    <div class="max-w-[1240px] mx-auto mt-[20px]">
      <div class="w-[80%] md:w-[700px] mx-auto">
        <client-only>
          <el-input v-model="searchKeyword" placeholder="请输入关键词搜索" @keydown.enter="search(searchKeyword)" prefix-icon="Search" size="large" input-style=" height: 48px;" clearable>
          </el-input>
        </client-only>
      </div>
    </div>
    <div class="mx-5 xl:max-w-[1200px] xl:mx-auto mt-[3.125rem] mb-[4.6875rem]" v-if="doubanData.length > 0">
      <h1 class="pl-3 text-[12px] sm:text-sm text-slate-600 font-bold dark:text-white mt-[20px]">豆瓣热门影视榜单</h1>
      <div class="grid grid-cols-2 xs:grid-cols-3 md:grid-cols-5 lg:grid-cols-5 xl:grid-cols-8 gap-3 mt-[10px]">
        <div class="mx-1 p-2 cursor-pointer truncate text-xs font-bold dark:bg-slate-700 dark:text-slate-100 rounded-[5px]" v-for="(movie, index) in doubanData" :key="index" type="info" @click="goDouban(movie)">
          <img class="w-full h-[180px] lg:h-[220px] xl:h-[161px] rounded-[5px] object-cover" :src="'https://images.weserv.nl/?url=' + movie.cover" alt="" referrerpolicy="never">
    ...
    <div class="py-2 h-25 fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 space-y-1">
      <div class="flex flex-row items-center justify-center gap-3">
        <a class="transition-opacity opacity-70 hover:opacity-100" href="https://github.com/unilei/aipan-netdisk-search">
          <img class="w-5 h-5" src="@/assets/skill-icons--github-dark.svg" alt="github">
        </a>
        <el-button class="transition-opacity opacity-70 relative" link color="#ffffff" @click="donate()">
          <img class="w-5 h-5 animate-ping absolute" src="@/assets/donation/dashang.svg" alt="打赏">
